...
In diesem Abschnitt wird die internen Projektverwaltung, die Werkzeugen zur Projektverwaltung und die Werkzeugen zur Projektrealisierung beschrieben.
...
\subsection{GitHub}
Zur kollaborative Versionsverwaltung wird der Online-Service \textbf{GitHub} verwendet. Welcher für OpenSource Projekte kostenlos öffentliche Repositorys zur Verfügung stellt. Da  das Projekt unter der \textbf{MIT-Lizenz} entwickelt wird ist die Nutzung eines öffentlichen Repositorys ein Vorteil in Hinsicht auf eine mögliche Weiterentwicklung und guter Akzeptanz in der OpenSource-Community.

Das GitHub Repository bietet die Möglichkeiten den Code, welcher lokal entwickelt wird, in das Repository hoch zu laden so das immer ein konsistenter gleicher Versionsstand vorhanden ist. Außerdem gibt es die Möglichkeit für einzelne Teilprojekte eigene \textbf{Branches} zu erstellen so das auf diesen Unabhängig vom Master-Branch, in welchem alle Teile zusammen fließen, entwickelt werden kann.

Zur Fehlerverwaltung wird das bereits Integrierte \textbf{Issue System} von GitHub verwendet. Mit diesem ist es möglich die Issues zu Priorisieren, mit Tags zu versehen, einzelnen Projektmitgliedern zuzuweisen und den Bearbeitungsstand zu Dokumentieren. Der weitere Vorteil hiervon ist das jedes Projektmitglied automatisch eine Benachrichtigung über neue Issues und Aktualisierungen erhält um somit bei der Fehleranalyse helfen kann.
Außerdem wurden einzelne \textbf{Milestones} erstellt welche sich an den fortlaufenden Besprechungen Orientierten und somit laufend Ziele bei der Projektdurchführung definierten. Nach der Erreichung großer Projektziele wurden Verssionsstände als \textbf{Releases} getaggt so das ersichtlich wurde welchen funktionellen Umfang der Code in diesem Stadium hat.

\subsection{CMake}
Da Ziel h) des Projekts ist plattformunabhängig zu sein wird eine Möglichkeit benötigt unterschiedliche \textbf{Integrierte Entwicklungsumgebungen (IDEs)} zu unterstützen.
Hierfür wurde das plattformunabhängige Build Werkzeug CMake, welches auch unter einer OpenSource Lizenz (BSD-Lizenz) erhältlich ist, verwendet. CMake erlaubt es für unterschiedliche IDEs Projektdateien zu erstellen. So das für die Entwicklung nicht eine spezifische Entwicklungsumgebung benötigt wird sondern mit mehreren auch Betriebssystem unabhängigen IDEs entwickelt werden kann.

\subsection{IDEs}
Als Integrierte Entwicklungsumgebung (IDEs) kamen \textbf{Visual Studio} unter Windows 7 und Windows 10 zum Einsatz. Zu Beginn des Projekts in Version Visual Studio 2013 in der zweiten Hälfte des Projekts dann in Version Visual Studio 2015, welches eine bessere Unterstützung für C++11 Sprachfeatures enthält. Unter Fedora 21 wurde \textbf{Netbeans} in Version 8 eingesetzt.

\subsection{Testumgebung}
Zum Testen des Codes wurden mehre unterschiedliche Testumgebungen eingesetzt. Dabei wurden\textbf{Rechnerübergreifende Tests} des Verbindungsaufbaus und der Verbindungsqualität über \textit{LAN}, \textit{WLAN} und \textit{WAN} mit verschiedenen Betriebssystemen durchgeführt. 
...
Für einen automatisierten Build-Test wurde auf den Online-Service \textbf{Travis CI}, welcher durch seine einfache Integration in das GitHub Repository eine gute Lösung bietet, gesetzt. Er bietet die Möglichkeit schnell herauszufinden ob der neu eingecheckte Code fehlerfrei unter \textit{Linux} und \textit{OSX} Compiliert.
...
Zum Erstellen des Projekt unter Windows wird eine aktuelle Version von \textbf{CMake} und die zu benutzende \textbf{Entwicklungsumgebung} welche von CMake unterstützt wird benötigt.
...
\begin{figure}[htp]
\centering
\includegraphics[width=.50\textwidth]{images/CMake}
...
Anschließend wir ausgewählt welche Audio Bibliothek verwendet wird, unter Windows wird \textit{ASIO}, \textit{DS} und \textit{WASAPI} unterstützt. Wahlweise kann auch \textit{CppTest} aktiviert werden falls Tests durchführen werden sollen.
Die Einstellungen werden durch nochmaligen druck auf \texttt{Configure} bestätigt und über \texttt{Generate} werden die Projekt Dateien im vorher definierten Build Verzeichnis erstellt.

Es wird die \textit{Entwicklungsumgebung} mit den generierten Projekt Dateien gestartet und angewiesen das Projekt zu Compilieren.

Im Verzeichnis \texttt{build/Release/} findet sich bei der Verwendung von Visual Studio und Compilierung mit Release Optionen das ausführbare Programm \textbf{OHMComm.exe} und die OHMComm Bibliothek \textbf{OHMComm.lib}.

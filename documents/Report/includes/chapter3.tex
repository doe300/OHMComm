...
In diesem Abschnitt werden wir uns näher mit der internen Projektverwaltung, den Werkzeugen zur Projektverwaltung und den Werkzeugen zur Projektrealisierung auseinandersetzen.
Die Aufgabenstellung und der Fortschritt des Projekts wurde fortlaufen in Treffen mit \textit{Herrn Professor Teßmann} besprochen. Zwischen den Treffen stand uns \textit{Herr Professor Teßmann} natürlich jederzeit für Persönliche und Schriftliche Fragen zur Verfügung.
\subsection{GitHub}
Um eine möglichst einfache kollaborative Versionsverwaltung zu ermöglichen entschieden wir uns für den Online-Service \textbf{GitHub}. Welcher für OpenSource Projekte kostenlos öffentliche Repositorys zur Verfügung stellt. Da wir uns dazu entschlossen hatten das Projekt unter der \textbf{MIT-Lizenz} zu entwickeln schien uns die Nutzung eines öffentlichen Repositories somit für eine geeignete Wahl, auch in Hinsicht auf eine mögliche Weiterentwicklung und gute Akzeptanz in der OpenSource-Community nach Ende des IT-Projektes.

Das GitHub Repository bietet uns die Möglichkeiten unseren Code welchen wir lokal Entwickeln in das Repository hoch zu laden so das wir immer auf einen konsistenten gleichen Versionsstand arbeiten. Außerdem gibt es die Möglichkeit für einzelne Teilprojekte eigene \textbf{Branches} zu erstellen so das auf diesen Unabhängig vom Master-Branch, in welchem alle Teile zusammen fließen, entwickelt werden kann. So ist z.B. die Opus Implementierung in einem eigenen Branch entstanden und wurde nach erfolgreichen Tests in den Master-Branch übernommen.

Zur Fehlerverwaltung wird das bereits Integrierte \textbf{Issue System} von GitHub verwendet. Mit diesem ist es uns möglich die Issues zu Priorisieren, mit Tags zu versehen, einzelnen Projektmitgliedern zuzuweisen und den Bearbeitungsstand zu Dokumentieren. Der weitere Vorteil hiervon ist das jedes Projektmitglied automatisch eine Benachrichtigung über neue Issues und Aktualisierungen erhält um somit bei der Fehleranalyse zu helfen.
Außerdem wurden einzelne \textbf{Milestones} erstellt welche sich an den fortlaufenden Besprechungen Orientierten und somit laufend Ziele bei der Projektdurchführung definierten. Nach der Erreichung großer Projektziele wurden Versionsstände als \textbf{Releases} getaggt so das ersichtlich wurde welchen funktionellen Umfang der Code in diesem Stadium hat.
\subsection{CMake}
Da eines der Ziele des Projekts ist platformunabhängig zu sein benötigen wir auch eine möglichkeit unterschiedliche \textbf{Integrierte Entwicklungsumgebungen (IDEs)} zu unterstützen.
Hierfür wurde das platformunabhängige Build Werkzeug CMake, welches auch unter einer OpenSource Lizenz (BSD-Lizenz) erhältlich ist, verwendet. CMake erlaubt es uns für unterschiedliche IDEs Projektdateien zu erstellen. So das wir uns für die Entwicklung nicht für eine spezifische Entwicklungsumgebung entscheiden müssen sondern mit mehreren auch Betriebssystem unabhängigen IDEs entwickeln können.

\subsection{IDEs}
Als Integrierte Entwicklungsumgebungen (IDEs) setzen wir auf \textbf{Visual Studio} unter Windows 7 und Windows 10. Zu Beginn des Projekts noch in Version Visual Studio 2013 in der zweiten hälfte des Projekts wurde dann auf Visual Studio 2015 upgegraded, welches eine bessere Unterstützung für C++11 Sprachfeatures enthält. Unter Fedora 21 setzten wir auf \textbf{Netbeans} in Version 8.

\subsection{Testumgebung}
Zum Testen des Codes setzten wir auf mehre unterschiedliche Testumgebungen. Dabei wurden auch \textbf{Rechnerübergreifende Tests} des Verbindungsaufbaus und der Verbindungsqualität über \textit{LAN}, \textit{WLAN} und \textit{WAN} mit verschiedenen Betriebssystemen durchgeführt. 

Zum Testen einzelner Module des Projektes setzen wir auf die Unit Test Umgebung \textbf{CPPTest}, welche uns ermöglicht einzelne Module auf ihre funktionalen Anforderungen hin zu überprüfen. 

Für einen automatisierten Build-Test setzen wir auf den Online-Service \textbf{Travis CI}, welcher durch seine einfache Integration in das GitHub Repository eine gute Lösung bietet schnell herauszufinden ob der neu eingecheckte Code fehlerfrei unter \textit{Linux} und \textit{OSX} Compiliert.

\section{Build Prozess}
\subsection{CMake}
Die CMake Konfiguration basiert auf mehreren \texttt{CMakeLists.txt} Dateien welche \textbf{hierarchisch} in die Projektstruktur integriert sind. In der Konfigurationsdatei auf der obersten Ebene wird die Minimal benötigte CMake \textit{Version} und der \textit{Projektname}(OHMComm) definiert.
Außerdem werden IDE abhängige Einstellungen vorgenommen wie z.B. die Einstellung des \textit{Warning Levels} für Compile Fehler und die Unterstützung von \textit{Unicode} Zeichen unter Visual Studio.
Anschließend werden die relativen Pfade in welchen die fertig Compilierten Dateien abgelegt werden definiert. Und die unteren Ordnerstrukturen mit ihren dort befindlichen Konfigurationsdateien angegeben.

In diesen weiteren Konfigurationsdateien für \textbf{OHMComm} und den extern benutzen Softwarekomponenten wie \textbf{Rtaudio} und \textbf{Opus} wird nun festgelegt welche \textit{Header} und welche \textit{C++ Dateien} eingebunden werden. 
...
Zum Erstellen des Projekt unter Windows wird eine aktuelle Version von \textbf{CMake} und natürlich die Lieblings \textbf{Entwicklungsumgebung} welche von CMake unterstützt wird benötigt.
Für einen sauberen Entwicklungsprozess ist es zu empfehlen sich zu erst ein Build Verzeichnis außerhalb des Sourcecodes anzulegen (Out-of-Source Build).

\begin{figure}[htp]
\centering
\includegraphics[width=.55\textwidth]{images/CMake}
\caption{CMake Konfiguration unter Windows 7}
\label{Fig:CMake}
\end{figure}

Nun teilen wir CMake mit wo sich unser Source Code \texttt{Browse Source} und unser Build Verzeichniss \texttt{Browse Build} befinden. 
Mittels \texttt{Configure} Button definieren wir unsere Entwicklungsumgebung und den verwendeten Compiler.
Anschließend wählen wir aus welche Audio Bibliothek wir verwenden möchten, unter Windows wird \textit{ASIO}, \textit{DS} und \textit{WASAPI} unterstützt. Wahlweise können wir auch noch \textit{CppTest} aktivieren falls wir damit Tests durchführen möchten.
Wir bestätigen unsere Einstellungen durch nochmaligen druck auf \texttt{Configure} und generieren über \texttt{Generate} unsere Projekt Dateien im vorher definierten Build Verzeichnis.

Wir starten unsere \textit{Entwicklungsumgebung} mit den generierten Projekt Dateien und weisen sie an das Projekt zu Compilieren.

Im Verzeichnis \texttt{build/Release/} finden wir nun bei der Verwendung von Visual Studio und Compilierung mit Release Optionen das auführbare Programm \textbf{OHMComm.exe} und die OHMComm Bibliothek \textbf{OHMComm.lib}.
...
Die Klasse bietet drei virtuelle Methoden, welche eine automatische Verarbeitung starten. Die Methode \texttt{startRecordingMode()} startet die Verarbeitung von Audioinputdaten (Mikrofon), \texttt{startPlaybackMode()} startet die Verarbeitung von Audiooutputdaten (Lautsprecher) und \texttt{startDuplexMode()} ist die gleichzeitige Verarbeitung von Audioinput- und Audioutputdaten. Letztere stellt den Standardfall für die Audiokommunikation dar. Wie diese Methoden im Detail aussehen, wird in Unterklassen von \texttt{AudioHandler} implementiert. Für die Hardwareansteuerung ist sinnvoll bereits existierde Lösungen zu verwenden. In diesem Fall kann man Unterklassen von \texttt{AudioHandler} auch als Wrapper-Klasse ansehen.

\subsubsection{Konfiguration}
Vor der Verarbeitung muss die Hardware konfiguriert werden. Hierfür kann das Struct \texttt{AudioConfiguration} verwendet werden. In diesem lassen sich das Mikrofon und die Lautsprecher bestimmen, die Anzahl der Kanäle (Mono - Stereo), die Sample Rate und die Größe des internes Buffers. Der interne Buffer kann vorerst vernachlässigt werden. Im Kapitel von \ref{sec:RTAudio} TODO-Kreuzreferenz wird dieser näher erläutert. Für die Übergabe der Einstellungen steht in der Klasse \texttt{AudioHandler} die Methode \texttt{setConfiguration()} zur Verfügung. Falls keine Konfiguration übergeben wird, so wird automatisch die virtuelle Methode \texttt{setDefaultAudioConfig()} aufgerufen. Diese muss ebenfalls von der Unterklasse implementiert werden. Die Methode stellt eine Standardkonfiguration zur Verfügung, jedoch ist zu beachten, dass diese Konfiguration eventuell fehlerhaft und nicht auf allen Systemen lauffähig ist.

\subsubsection{Steuerung der Verarbeitung}
Falls die Verarbeitung läuft kann diese durch Methoden gesteuert werden. Die Methode \texttt{suspend()} pausiert die aktuelle Verarbeitung und \texttt{resume()} setzt sie weiter fort. Die Methode \texttt{stop()} stoppt die Verarbeitung komplett und kann nicht durch \texttt{resume()} fortgesetzt werden. \texttt{reset()} stoppt die Verarbeitung und setzt die übergebene Audiokonfiguration wieder zurück.

\subsubsection{Prepare-Methode}
Die virtuelle Methode \texttt{prepare} nimmt eine Sonderrolle ein. Grundsätzlich kann Sie als optional betrachtet werden, jedoch ist sie für bestimmte Einsatzzwecke sinnvoll. Sie sollte aufgerufen nachdem eine Audiokonfiguration übergeben wurde und bevor die Verarbeitung gestartet ist. Dies kann sinnvoll sein um z.B. die ausgewählte Hardware im Hinblick auf bestimmte Einstellungsmöglichkeiten und deren Kompatiblität mit anderen Komponenten zu überprüfen. Falls eine Inkompatiblität festgestellt wird, kann der Start der Verarbeitung abgebrochen wurden.
...
Das Implementieren der Klasse \texttt{AudioProcessor} reicht nicht aus, um an der Audioverarbeitung aktiv teilzunehmen. Abbildung \ref{Fig:AudioHandlerAudioProcessor} zeigt die Beziehung zwischen \texttt{Audio"-Handler} und \texttt{Audio"-Processor}. Die Klasse muss sich beim \texttt{AudioHandler} mit der Methode \texttt{addProcessor()} anmelden. Diese nimmt als Parameter einen \texttt{AudioProcessor} entgegen. Audioprozessoren lassen sich über ihren Namen eindeutig identifizieren. Der Prozessor wird, falls er schon nicht vorhanden ist, in einer Liste vom Typ \texttt{AudioProcessor} eingefügt. Falls nun Audioinputdaten anliegen, so werden von allen Audioprozessoren, die sich in der Liste befinden, die \texttt{process"-Input"-Data()}-Methoden aufgerufen. Die Aufrufreihenfolge ist abhängig von der Anmeldereihenfolge. Für Audiooutput-Daten geschieht das analog, jedoch ist hier die Aufrufreihenfolge entgegengesetzt, da die Verarbeitungskette nicht kommutativ ist. Daraus folgt, dass die \texttt{process"-Output"-Data()}-Methoden genau in der umgekehrten Reihenfolge aufgerufen werden müssen, um wieder an die ursprünglichen Audiodaten zugelangen. Es lassen sich beliebige viele Prozessoren beim \texttt{AudioHandler} anmelden. Jedoch ist zu beachten, dass die Performanz sinkt je mehr Prozessoren angemeldet sind. Vom Aufbau ähnelt die Verarbeitungskette, dem Observer-Pattern, jedoch mit dem entscheidenden Unterschied, dass die Einfügereihenfolge eine wesentliche Rolle für die Verarbeitung spielt. 
\newline
\begin{figure}[htp]
\centering

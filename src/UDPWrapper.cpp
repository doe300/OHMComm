...
int UDPWrapper::process(void* outputBuffer, void* inputBuffer, unsigned int nFrames, double streamTime, RtAudioStreamStatus status, void* userData)
{
    ...
    long unsigned int dataSize = nFrames * getBytesFromAudioFormat(audioConfiguration.InputAudioFormat);
    
    //send
    if(inputBuffer != NULL)
    {
        long int size = sendto(Socket, (char *)inputBuffer, dataSize, 0, &networkConfiguration.remoteAddr, sizeof(networkConfiguration.remoteAddr));
        if(size == -1)
        {
    ...
    {
        long int size = recvfrom(Socket, (char *)outputBuffer, dataSize, 0, NULL, 0);
        if(size == -1)
        {

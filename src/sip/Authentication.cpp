...
Authentication::Authentication() : isAuthenticated(false)
{
}
...
std::unique_ptr<Authentication> Authentication::getAuthenticationMethod(const std::string headerField)
{
    ...
    if(Utility::trim(headerField).find("Digest") == 0)
    {
        ...
        return std::unique_ptr<Authentication>(new DigestAuthentication(realm, nonce, algorithm));
    }
...
DigestAuthentication::DigestAuthentication(const std::string& realm, const std::string& nonce, const std::string& algorithm) : 
    Authentication(), realm(realm), nonce(nonce), algorithm(algorithm)
{
}
...
std::string DigestAuthentication::createAuthenticationHeader(const std::string& userName, const std::string& password)
{
    ...
    return Utility::joinStrings({"Digest ", "username=\"", userName, "\", realm=\"", realm, "\", nonce=\"", nonce, 
            "\", algorithm=", algorithm, ", response=\"TODO\""}, "");
}
